...
- **无行号整文件读取** `read_plain`：与 `str_replace_editor` 的 `view`（带行号）互补，原样返回文件全文，无行号、无截断指引、无编码味注释。相对路径按当前会话的工作目录（cwd）解析——与 `read` 等官方文件工具一致，而不是服务器进程的启动目录
...
| 工具 | 说明 |
|---|---|
| `read_plain`（新增） | 读取文本文件全部内容，**不带行号**；相对路径以会话工作目录（cwd）为基准；上限可配置（默认 512 KiB） |
